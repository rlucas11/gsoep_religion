...
As noted previously, the results from the full-sample analyses mostly replicate the average effects from the meta-analysis of results across the 14 federal states analyzed in @entringer_big_2023. By focusing on the full sample, comparison across models was simplified, and the effect of any state-specific model estimation problems could be eliminated. However, using this full-sample approach, we could not replicate the state-level contextual moderator effects, so we also conducted state-level analyses to examine how modeling choices affect state-level religiosity as a moderator of the lagged effects. 
