...
This paper uses data from the German Socio-Economic Panel (SOEP) study, which assessed the Big Five personality traits and religiosity four times, at four-year intervals. The inclusion of four waves of assessment allows for the use of both the CLPM and the more complex RI-CLPM. @entringer_big_2023 provided detailed code for their analyses, however, they did not provide code to extract and clean data from the raw data files. As a test of the reproducibility of the analyses from the description provided in the text, we developed our own code for extracting and cleaning data based on the decision rules reported in the text. We also relied on a more recent release of the dataset (Version 37, versus Version 35 used in the original paper), which includes two more waves of data. Thus, the exact number of participants included in the final samples, and the precise estimates from the original model could not be perfectly reproduced. However, both the final sample sizes and results are quite close to the original, and all substantive conclusions from the original paper were supported in analyses that use the same model. Moreover, the focus in this paper is in comparing analyses across different model specifications, all of which were able to be run using the updated data set. Thus we proceeded with our robustness checks using the sample we extracted. Our full code for extracting, cleaning, and analyzing these data is available at [https://github.com/rlucas11/gsoep_religion](https://github.com/rlucas11/gsoep_religion) (eventually replace this with permanent osf site). 
...
When tested using a single model with the full sample, the model specification from @entringer_big_2023 did result in one effect that was not significant in their analyses but was significant in ours. However, the estimate itself was virtually identical across the two analyses. Specifically, the estimate for the path from openness to religiosity was a nonsignificant -0.013 in the original analysis, versus a significant `r plotData %>% filter(trait == "opn", model == "clpm", type == "latent", variables == "all") %>% select(est) %>% round(3)` in ours. This is clearly not a consequential difference. 
